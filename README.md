...
**专业绘图宏包补全**：
- **CircuiTikZ**（`circuitikz`）：358 个路径元件 + 128 个 node 形状 + 全局键（`voltage`/`current`/`voltage dir`/`logic ports`/各类 `scale`/`mirror`/`invert`/`l`/`v`/`i`/`a` 标注键等）；锚点补全涵盖 `pin 1`–`pin 3`（flipflop/dipchip 的引脚触点）、`bpin 1`–`bpin 3`（引脚边框锚点）等动态创建锚点。
