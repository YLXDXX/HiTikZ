...
- **导入 .tex 文件**：支持导入单个 `.tex` 文件自动提取 TikZ 代码（`\begin{document}...\end{document}` 之间、`\begin{tikzpicture}...\end{tikzpicture}` 或全文回退），文件名自动作为片段名称
...
| `test_tex_import` | .tex 文件导入代码提取（document 体 / tikzpicture 环境 / 全文回退三段式解析） |
...
#### Bug 修复
- **scratch 模式错误行跳转**：修复编译错误后双击日志无法正确跳转到代码行的回归（`handleLogDoubleClick` 双重行号偏移）
- **scratch 模式参数替换失效**：修复未保存片段点击"编译预览"时 `@@var@@` 未被替换的问题
- **scratch 模式退出失效**：修复启动后点击缩略图加载片段时仍停留在 scratch 模式的问题（`addTab` 信号时序中 tab data 未就绪）
- **重复打开片段导致标签跳转**：修复创建新标签页时 `addTab` 同步触发 `onTabChanged` 读取空 tabData 覆盖 `currentSnippetId`
- **剪贴板 PNG/SVG 连接重复累积**：添加防重入守卫标志，修复快速多次点击复制按钮导致多回调叠加
- **缩放按钮状态不同步**：缩放操作后自动更新适应模式按钮的选中状态
- **重复 refreshSearch 调用**：移除批量操作中 `categoriesChanged` 信号触发的冗余刷新
- **重命名分类性能**：改为仅按需加载匹配片段的完整数据，避免全量代码读取
- **搜索空候选集处理**：bigram 索引交集为空时直接返回，避免不必要的全量片段扫描
- **SVG 剪贴板 UTF-8 BOM 保留**：使用原始字节传递文本 MIME 类型，避免 `QString::fromUtf8` / `toUtf8` 双向转换剥离 BOM

#### 新功能
- **草稿恢复机制**：自动保存间隔由 60 秒调整为 3 分钟，保存完整元数据；异常退出后重启弹出恢复对话框显示所有草稿名称，支持勾选恢复或全部丢弃
- **异步批量编译预览**：从阻塞式同步伪循环改为完全异步队列模式，状态栏实时进度，单片段 30 秒超时自动跳过错片
- **导入单个 .tex 文件**：工具栏新增菜单项，自动从 .tex 文件中提取 TikZ 代码（document 体 → tikzpicture 环境 → 全文三段式回退），文件名作为片段名称

#### 界面优化
- 统一导出菜单文案：导出为 Tex 文档 / 导出为 PDF 文档 / 导出为 PNG 图片 / 导出为 SVG 图片
- 撤销/重做按钮改用箭头符号 ↩ / ↪
- 缩放按钮改用数学减号 − / 加号 +
- "复制完整文档"简化为"复制文档"
- 工具栏按钮重新排序：撤销/重做移至编译/应用参数之前

#### 测试
- 新增 `test_draft_recovery`：草稿文件格式完整性测试（全字段 JSON 读写往返、空代码过滤、目录扫描）
- 新增 `test_tex_import`：.tex 文件导入代码提取三段式解析测试
- 新增 `test_params`：参数声明正则解析与 `@@var@@` 替换正确性测试（单参数、多参数、负数、零值）
- 所有测试通过（9/9，ctest）
