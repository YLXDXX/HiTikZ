...
| 坐标/节点名 | `(` 后 | 用户定义的坐标名、节点名（含 `name=<name>` 语法提取含空格的名称如 `critical 1`，以及 `name intersections={…,by={…}}` 中 `by=` 声明的交点名）；同时提示 TikZ 坐标系写法 `xyz cs:`/`canvas polar cs:` 等（详见下方「坐标系补全」） |
