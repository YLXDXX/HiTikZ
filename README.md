...
- **智能代码补全**：上下文感知的结构化补全，覆盖 TikZ/PGF 及多个专业绘图宏包，并自动纳入用户定义的样式、坐标、命令等（详见[代码补全](#代码补全)）
...
│                                    #（合计约 2000+ 条目：CircuiTikZ 元件394+形状128 + tkz-euclide 164 + chemfig + tikz-feynman + tikz-cd + 形状~120 + PGF键路径30+）
...
| `test_completer` | 补全词库完整性（去重、条目数量）、detectContext 10 种上下文检测（37 用例含 `\usetikzlibrary{}` 库补全）、PGF 键处理器 / 值提示 / `=` 颜色值补全、key 提取的花括号 / 中括号感知、跨行上下文回溯等；CircuiTikZ / 标准 TikZ / 数学函数 / 装饰 / physics·siunitx / pgfplots / tikz-cd·tkz-euclide·chemfig·tikz-feynman 各词库组的源码一致性校验，确保所有条目为对应宏包的真实命令/选项/形状，并仅由正确的库或环境门控 |
