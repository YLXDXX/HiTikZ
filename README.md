...
- **语法高亮**：TikZ/LaTeX 代码彩色高亮，覆盖 **15 条语法规则**（注释/命令/环境/数学/坐标/选项/字符串/数字/参数/括号/PGF路径/键处理器/TikZ库）及 **6 类用户定义名称**（样式/节点/坐标/foreach变量/自定义命令/自定义颜色），支持 `\begin{comment}...\end{comment}` 多行注释块。key=value 中 key 和 value 分色显示。**用户定义高亮保护**——用户样式名、坐标名等出现在注释中时不会覆盖注释的灰色斜体格式。**跨行选项高亮**——换行书写的 `[...]` 选项列表通过块状态机跟踪，每一行都正确高亮为选项色
- **选中词高亮**：光标置于某单词或双击选中时，文档中所有相同单词自动高亮，快速定位变量/命令的使用位置
- **智能代码补全**：**上下文感知补全**——根据当前所处环境（tikzpicture/scope/axis/circuitikz）、当前绘图命令（\draw/\node/\path/\fill）、已加载库自动过滤候选项（如 circuitikz 环境下显示电路选项，axis 环境下显示 pgfplots 选项，载入 shadows.blur 库时提示模糊阴影选项，未加载 decorations 库时不提示 decoration 相关选项）；**用户定义补全**——自动识别用户自定义样式（含空格名如 `test lines`）、节点名、坐标名、foreach 变量（支持空格分隔多变量如 `\foreach \xyz / \xtext in`）、自定义命令、自定义颜色、pic 名并纳入补全；**智能值提示**——`=` 后的值根据 key 名动态匹配（如 `color=`/`draw=`/`fill=` 提示完整内置调色板及用户 `\definecolor`/`\colorlet` 颜色，`pattern=` 仅提示图案）；覆盖 **~1400+ 个结构化关键词**（含 ~200 CircuiTikZ 组件选项、90 双极前缀组合、72 tkz-euclide v5 命令）；CircuiTikZ 组件通过 `requiredLibs` 库过滤，仅在激活时提示；片段元数据中的 TikZ 库自动传递给补全系统；`\usepackage{circuitikz}` 和 `\usepackage{tkz-euclide}` 自动检测激活对应库；**跨行上下文回溯**——补全上下文向前回溯到最近未闭合的 `[`/`{`，换行书写的多行选项列表内也能正常补全
...
| 值 | `=` 后 | **智能过滤**：按 key 名匹配——颜色键（`color`/`draw`/`fill`/`left color`/`ball color` 等）提示完整内置调色板及用户 `\definecolor`/`\colorlet` 颜色、宽度键仅提示尺寸、`pattern=` 仅提示图案、`decoration=` 仅提示装饰 |
| 参数 | `@@` 未闭合 | 代码中声明的参数变量 |
...
├── latex_compiler_extract.cpp       # 自定义命令抽取（40+ 类定义命令）
...
├── tikz_completer.h / tikz_completer.cpp # 核心：补全触发、按键处理
...
├── tikz_keywords_commands.cpp       # 命令 + CircuiTikZ 组件 + 双极前缀 + tkz-euclide + 数学函数
├── tikz_keywords_options.cpp        # 通用选项
├── tikz_keywords_pgfplots.cpp       # 3dplot / pgfplots / 杂项形状选项
├── tikz_keywords_extended.cpp       # CircuiTikZ 形状/选项、tikz-cd、graphs、pgfplots 扩展等
│                                    #（合计 ~1400+ 条目：CircuiTikZ 组件~200 + 双极前缀90 + 形状~120 + PGF键路径30+ + tkz-euclide v5 72 命令）
...
| `test_completer` | TikZ 补全词库完整性验证（选项/锚点/颜色/线型/交点/角度库关键字含重复检测）、detectContext 上下文检测（34 个测试用例）、PGF 键处理器、值提示（箭头/图案/角度/曲线）、`=` 后颜色值补全（`fill`/`draw`/`color` 提供完整调色板，非颜色键保留精选值）、`=` 后 key 名花括号/中括号感知提取、同行多命令取最内层命令、跨行上下文回溯、非法命令过滤（选项/环境/路径操作/CircuiTikZ 组件不作为 `\命令`）、空列表模型清空（20 项测试） |
