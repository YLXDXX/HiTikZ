...
- **树形分类**：支持层级分类（如 `数学/几何`），含「全部」与「未分类」节点；分类树直接显示片段名称；支持空分类；拖拽片段到分类节点即可重新归类；拖拽排序持久化到 `sortOrder`（double 类型，新建片段默认排在该分类最前）
...
- **多选与批量操作**：Ctrl+点击多选缩略图，右键菜单：复制 / 删除 / 批量导出；分类树 Ctrl+点击多选分类与片段节点，右键菜单：删除 / 批量导出分类
- **属性编辑**：右键单个缩略图弹出属性对话框，编辑全部元数据字段（含自定义编译引擎）
- **片段复制**：工具栏复制当前片段；右键缩略图或分类树片段节点可后台复制
...
- **智能代码补全**：上下文感知的结构化补全，覆盖 TikZ/PGF 及多个专业绘图宏包，13 种上下文 + 三级过滤，并自动纳入用户定义的样式、坐标、命令等（详见[代码补全](#代码补全)）；`\begin{env}` 补全后自动插入 `\end{env}`
...
- **括号配对高亮**：可选（设置 → 行为设置，默认关闭）。开启后光标在 `{}`、`[]`、`()` 旁边时高亮显示与之配对的括号（蓝色背景）；找不到配对括号时以红色背景警示；选中单个括号字符时同样触发高亮。支持跨行嵌套、混合括号类型。采用逐字符深度追踪算法（非正则），跨行性能稳定
- **过长行自动换行**：可选（默认开启），过长代码行软换行显示无需横向滚动，续行在行号栏以 `↳` 标记
- **撤销/重做**：Ctrl+Z / Ctrl+Shift+Z，工具栏图标按钮；无内容可撤销/重做时按钮自动置灰
...
- **宏包与 TikZ 库**：每个片段可声明额外 `\usepackage` 宏包与 `\usetikzlibrary` 库，编译时自动注入导言区；右下角「额外宏包」「TikZ库」两栏均带逗号感知的名称自动补全（弹窗以琥珀色/蓝色强调色着色并带彩色圆点标记，一眼可辨），且库栏改动即时同步到编辑器补全（无需在代码里写 `\usetikzlibrary`）
...
- **开机自启动**：设置面板一键开关；自启动以 `--hidden` 方式静默驻留托盘，登录后不弹出主窗口（旧的自启动条目会在程序启动时自动升级补上 `--hidden`）
...
| 测试 | 内容 |
|------|------|
| `test_snippet_manager` | 片段创建/读取/更新/删除（含空ID防御），loadCode，renameCategory，compileCommand/sortOrder JSON 序列化，ZIP 导入/导出，reorderSnippets，categoryOrder 保存/加载 |
