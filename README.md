...
- **语法高亮**：TikZ/LaTeX 代码彩色高亮，覆盖 **15 条语法规则**（注释/命令/环境/数学/坐标/选项/字符串/数字/参数/括号/PGF路径/键处理器/TikZ库）及 **6 类用户定义名称**（样式/节点/坐标/foreach变量/自定义命令/自定义颜色），支持 `\begin{comment}...\end{comment}` 多行注释块。key=value 中 key 和 value 分色显示。**用户定义高亮保护**——用户样式名、坐标名等出现在注释中时不会覆盖注释的灰色斜体格式
