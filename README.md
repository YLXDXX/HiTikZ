...
- **语法高亮**：TikZ/LaTeX 代码彩色高亮，覆盖 **21 类语法元素**（命令/环境/注释/坐标/数学/选项/字符串/数字/参数/PGF路径/键处理器/TikZ库）及 **6 类用户定义名称**（样式/节点/坐标/foreach变量/自定义命令/自定义颜色），支持 `\begin{comment}...\end{comment}` 多行注释块。key=value 中 key 和 value 分色显示
- **选中词高亮**：光标置于某单词或双击选中时，文档中所有相同单词自动高亮，快速定位变量/命令的使用位置
- **智能代码补全**：**上下文感知补全**——根据当前所处环境（tikzpicture/scope/axis/circuitikz）、当前绘图命令（\draw/\node/\path/\fill）、已加载库自动过滤候选项（如 circuitikz 环境内只显示电路选项，axis 环境内显示 pgfplots 选项，载入 shadows.blur 库时提示模糊阴影选项，未加载 decorations 库时不提示 decoration 相关选项）；**用户定义补全**——自动识别用户自定义样式（含空格名如 `test lines`）、节点名、坐标名、foreach 变量、自定义命令、自定义颜色、pic 名并纳入补全；值提示根据当前 key 名动态匹配；覆盖 ~1100 个结构化关键词（含环境/命令/库元数据）；片段元数据中的 TikZ 库自动传递给补全系统
...
智能补全（`TikzCompleter`）基于**结构化词库**（`TikzKeywordDB`，~1100 条带环境/命令/库元数据的结构化条目）和**文档状态追踪**（`TikzDocumentState`，范围栈/用户定义名/活动库），实现三层精细过滤：

**基础上下文**（10 种）：
| 上下文 | 触发条件 | 补全内容 |
|--------|---------|---------|
| 命令 | 输入 `\` 后接字母 | ~317 个 TikZ/LaTeX/PGF 命令 |
| 环境 | `\begin{` 未闭合 | ~65 个 LaTeX/TikZ 环境 |
| 选项 | `[...]` 内 | 环境/命令/库三级过滤的选项（按需出现） |
| 锚点/键处理 | 单词后跟 `.` | ~55 个锚点 + 16 个 PGF 键处理器 |
| 值 | `=` 后 | 根据当前 key 名动态匹配值提示 |
| 参数 | `@@` 未闭合 | 代码中声明的参数变量 |
| TikZ 库 | `\usetikzlibrary{` 内 | ~88 个 TikZ 库名 |
| 通用词 | 任意已知词 | ~1145 个所有词表去重并集 |
...
**库过滤**：未加载 `decorations.pathmorphing` 库时不提示 `snake`/`coil`/`zigzag`；未加载 `angles` 库时不提示 `angle radius`/`angle eccentricity`；未加载 `shadows.blur` 库时不提示 `blur shadow` 等模糊阴影选项；未加载 `fadings` 库时不提示 `path fading` 等渐变透明选项。片段的 TikZ 库元数据自动注入。
...
- foreach 变量：`\foreach \x in ...`、`\foreach \xyz / \xtext in ...` → 命令上下文（`\x`、`\xyz`、`\xtext`）
...
├── tikz_keywords.h / .cpp           # 结构化关键词库（TikzKeywordDB 单例，~1100 条目含环境/命令/库元数据，新增执行钩子/混合模式/标注形状/装饰/fadings/shadows.blur/pgfplots/CircuitikZ/tikz-cd/graphs 等类别）
