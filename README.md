...
| `test_completer` | TikZ 补全词库完整性验证（选项/锚点/颜色/线型/交点/角度库关键字含重复检测）、detectContext 上下文检测（34 个测试用例）、PGF 键处理器、值提示（箭头/图案/角度/曲线）、`=` 后颜色值补全（`fill`/`draw`/`color` 提供完整调色板，非颜色键保留精选值）、`=` 后 key 名花括号/中括号感知提取、同行多命令取最内层命令、空列表模型清空（18 项测试） |
