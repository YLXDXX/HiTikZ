...
| 选项 | `[...]` 内 | ~360 个 TikZ 选项 | `thick`、`left color`、`help lines`、`even odd rule`、`angle radius` |
| 锚点/键处理 | 单词后跟 `.` | ~55 个锚点 + 16 个 PGF 键处理 | `north`、`south east`、`.style`、`.default`、`.code` |
| 值 | `=` 后（持续到值输入完毕） | 颜色 / 线型 / 箭头 / 角度 / 长度等值提示 | 输入 `color=` 后列出全部颜色名；`in=`/`out=` 列出常用角度 |
...
| 通用词 | 输入任意已知词 | ~450+ 命令/选项/锚点/颜色/线型/关键字的并集 | 连续输入 2 个以上字母时触发 |
| 点续写 | `.` 后继续输入（如 `.def`） | 锚点 + 键处理器持续补全 | `node.east`、`test lines/.style` |

**补全交互**：
- 自动弹出：检测到对应上下文时自动显示补全列表；括号 `[` 或 `.` 打开时立即显示全部候选项
- 键值延续：输入 `=` 后继续输入值时保持值补全上下文（如 `color=r` → `color=red`）
- 默认选中第一个候选项
- ↑↓ 键切换选中项，Enter / Tab 上屏
- Esc 关闭列表
- 候选词已去除重复项（如之前由于标准区和 CircuitikZ 区重复导致的 `east` 等锚点出现两次）

**补全覆盖**：
- **括号选项**：~360 个选项，涵盖基本绘图、定位、颜色、线型（含 `densely dashed`/`densely dotted` 等 12 种完整线型）、填充规则（`nonzero rule`/`even odd rule`）、图案、装饰、图形变换、pgfplots 坐标轴、3D 视图、节点形状（含 `isosceles triangle`/`semicircle`/`single arrow` 等几何形状）、预定义样式（`help lines`/`information text` 等）、路径构造关键字（`controls`/`parabola`/`cycle`）、19 种直接颜色键
- **位置与曲线**：`near start`/`near end`/`very near start`/`very near end`/`at start`/`at end` 定位，`in`/`out`/`in looseness`/`out looseness` 曲线角度
- **intersections 库**：`name path`、`name intersections`、`of`、`by`、`sort by`、`total`
- **angles 库**：`angle radius`、`angle eccentricity`、`right angle`、`pic text`
...
└── test_completer.cpp               # 补全词库完整性 + detectContext 上下文检测（30+ 用例）
```
...
| `test_completer` | TikZ 补全词库完整性验证（选项/锚点/颜色/线型/交点/角度库关键字含重复检测）、detectContext 上下文检测（30+ 测试用例含括号/点/键值续写）、PGF 键处理器、值提示（箭头/图案/角度/曲线） |
