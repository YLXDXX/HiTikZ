...
- **语法高亮**：TikZ/LaTeX 代码彩色高亮，覆盖 **14 条正则规则 + 跨行选项括号处理 + 6 类用户定义名称**。支持 `\begin{comment}...\end{comment}` 多行注释块。key=value 中 key 和 value 分色显示，`=` 嵌套在 `{}` 值内部时不会被误标为键名。**跨行选项高亮**——换行书写的 `[...]` 选项列表通过块状态机跟踪（含花括号深度感知），每一行正确高亮；跨行 key=value 的键名也通过状态机保持识别。**用户定义高亮保护**——用户样式名、坐标名等出现在注释中时不会覆盖注释格式
- **选中词高亮**：光标置于某单词或双击选中时，文档中所有相同单词自动高亮，快速定位变量/命令的使用位置
- **智能代码补全**：**上下文感知补全**——根据当前所处环境（tikzpicture/scope/axis/circuitikz）、当前绘图命令（`\draw`/`\node`/`\path`/`\fill`）、已加载库自动过滤候选项；**用户定义补全**——自动识别用户自定义样式（含空格名）、节点名、坐标名、foreach 变量（支持空格分隔多变量及可选方括号参数 `[count=\i]`）、自定义命令、颜色、pic 名并纳入补全；**智能值提示**——`=` 后根据 key 名动态匹配（颜色键提示完整调色板，`pattern=` 仅提示图案），key 名提取感知花括号/中括号嵌套；**命令触发**——`\` 前为 `{`/`(` 等非字母也可触发补全；**非法命令过滤**——路径操作/选项/环境/CircuiTikZ 组件等不会被误当成 `\命令` 提示；覆盖 **~1700+ 个结构化关键词**（CircuiTikZ 路径元件 394 + CircuiTikZ node 形状 128 + physics 包 ~130 命令 + siunitx 命令 + tkz-euclide 5.10c 165 命令 + chemfig/tikz-feynman/tikz-cd 补全，全部从对应宏包源码提取）；**跨行上下文回溯**——补全上下文向前回溯到最近未闭合的 `[`/`{`，换行选项列表内也能正常补全
- **自动缩进**：换行时继承上一行缩进，`{` 或 `\begin` 结尾的行自动增加一级缩进
- **过长行自动换行**：可在设置中开启（默认开启），过长代码行在编辑器内软换行显示，无需横向滚动；行号栏对折行产生的续行以 `↳` 标记，用户一眼即可区分「新的一行」与「上一行折下来的续行」
...
智能补全（`TikzCompleter`）基于**结构化词库**（`TikzKeywordDB`，~1700+ 条带环境/命令/库元数据的结构化条目）和**文档状态追踪**（`TikzDocumentState`，范围栈/用户定义名/活动库/`\usepackage` 检测），实现三层精细过滤。补全上下文检测采用**跨行回溯**——从光标向前回溯到最近未闭合的 `[` 或 `{`（带回溯上限），因此换行书写的选项 / 参数也能正确识别上下文并补全：

**基础上下文**（10 种）：
| 上下文 | 触发条件 | 补全内容 |
|--------|---------|---------|
| 命令 | 输入 `\` 后接字母（`\` 前为 `{`/`(`/`[` 等非字母也可触发） | 按当前环境和激活库动态过滤的 TikZ/LaTeX/PGF/tkz-euclide 命令（不含路径操作/选项/环境等非法 `\` 命令） |
| 环境 | `\begin{` 未闭合 | ~65 个 LaTeX/TikZ 环境 |
| 选项 | `[...]` 内 | 环境/命令/库三级过滤的选项，含 394 个 CircuiTikZ 路径元件（`R`/`C`/`pR`/`vR`/`rmeterwa` 等，含前缀快捷方式，均从 CircuiTikZ 1.7.1 源码提取，无拼写错误/无 `shape` 后缀垃圾项） |
| 锚点/键处理 | 字母或 `/` 后跟 `.` | ~55 个锚点 + 16 个 PGF 键处理器（不含小数点数字触发） |
| 值 | `=` 后 | **智能过滤 + 花括号感知**：`=` 查找忽略 `{}` 嵌套（`{a=b}` 不误触发），按 key 名匹配——颜色键提示完整调色板及用户颜色，`pattern=` 仅提示图案，`decoration=` 仅提示装饰 |
| 参数 | `@@` 未闭合 | 代码中声明的参数变量 |
| TikZ 库 | `\usetikzlibrary{` 内 | ~88 个 TikZ 库名（从词库动态读取） |
| 通用词 | 任意已知词 | ~1700+ 个所有词表去重并集 |
...
**数学物理宏包补全**：`\usepackage{physics}` 激活 physics 包约 130 个命令（`\vb`/`\va`/`\vu`/`\dv`/`\pdv`/`\grad`/`\div`/`\curl`/`\laplacian`/`\qty`/`\bra`/`\ket`/`\braket`/`\expval`/`\comm`/`\dd`/`\cross`/`\norm`/`\tr` 等，均从 `physics.sty` 源码提取）；`\usepackage{siunitx}` 激活 siunitx 命令（`\SI`/`\si`/`\num`/`\ang`/`\qty`/`\unit`/`\numlist`/`\qtyrange`/`\sisetup` 等）；`\usepackage{pgfplots}` 激活 pgfplots 键。这些命令仅在对应宏包被使用时才提示（编辑器 `\usepackage` 或片段「额外宏包」字段均可触发），不污染纯 TikZ 补全。

**专业绘图宏包补全**（均源码核对）：
- **CircuiTikZ**（`circuitikz`）：394 个路径元件 + 128 个 node 形状 + 全局键（`voltage`/`current`/`voltage dir`/`logic ports`/各类 `scale`/`mirror`/`invert`/`l`/`v`/`i`/`a` 标注键等），源自 CircuiTikZ 1.7.1。
- **tkz-euclide**（`tkz-euclide`）：165 个 v5.10c 命令（定义点/线/圆/多边形/三角形、绘制、交点、角标注、变换等），已清除 v5 不存在的 v4 旧命令（`\tkzDrawTriangle`/`\tkzDrawSquare`/`\tkzTangent` 等）。
- **tikz-cd**（`tikzcd` 环境）：箭头方向快捷命令（`\ar`/`\rar`/`\lar`/`\uar`/`\dar`/`\urar`/`\ular`/`\drar`/`\dlar`）+ 箭头样式键（`rightarrow`/`hook`/`harpoon'`/`two heads`/`maps to`/`dashed`/`squiggly`/`equal` 等）+ 图表选项（`row sep`/`column sep`/`crossing over`/`phantom` 等），源自 `tikzlibrarycd.code.tex`；已清除幻觉项 `obj`/`tweak`/`math nodes`。
- **chemfig**（`chemfig`）：40 个命令（`\chemfig`/`\definesubmol`/`\chemname`/`\chemabove`/`\schemestart`/`\charge`/`\polymerdelim` 等）+ `\setchemfig` 键（`atom sep`/`bond offset`/`double bond sep`/`angle increment`/`cram width`/`arrow offset` 等），源自 chemfig 1.66。
- **tikz-feynman**（`tikz-feynman`）：命令（`\feynmandiagram`/`\vertex`/`\diagram`/`\feynman`）+ 粒子/边样式（`fermion`/`anti fermion`/`photon`/`boson`/`scalar`/`ghost`/`gluon`/`majorana`/`momentum`/`half left` 等），源自 tikz-feynman 1.1.0。

**全部条目均以「本地 TeX Live 源码逐条核对」方式验证**（PGF/TikZ 3.1.10、pgfplots 1.18.1、CircuiTikZ 1.7.1、tkz-euclide 5.10c、tikz-cd、chemfig 1.66、tikz-feynman 1.1.0、physics、siunitx），清除了 `polygon`/`circle around` 等幻觉形状、`cit`/`kil`/`vecilen`/`power` 等幻觉数学函数、`curly brace`/`Hilbert curve` 等幻觉装饰、`fontsize`/`information text`/`branch` 等幻觉选项、`xstep`/`bar gap`/`histogram` 等错误 pgfplots 键。
...
│
├── code_editor.h / .cpp             # 编辑器（行号+续行 ↳ 标记、当前行/选中词高亮、自动缩进、过长行换行、补全/高亮器集成）
...
├── tikz_completer_context.cpp       # 上下文检测（10 种上下文）
...
├── tikz_keywords_commands.cpp       # 命令 + CircuiTikZ 路径元件（394 个，源自 CircuiTikZ 1.7.1）+ physics/siunitx 命令 + tkz-euclide + 数学函数（源自 PGF 数学引擎）
├── tikz_keywords_options.cpp        # 通用选项
├── tikz_keywords_pgfplots.cpp       # 3dplot / pgfplots / 杂项形状选项
├── tikz_keywords_extended.cpp       # CircuiTikZ 形状/选项、tikz-cd、chemfig、tikz-feynman、graphs、pgfplots 扩展等（均源码核对）
│                                    #（合计 ~1700+ 条目：CircuiTikZ 元件394+形状128 + tkz-euclide 165 + chemfig + tikz-feynman + tikz-cd + 形状~120 + PGF键路径30+）
├── tikz_document_state.h / .cpp     # 文档状态追踪（范围栈、库解析、`\usepackage` → 活动库映射（circuitikz/tkz-euclide/tikz-cd/chemfig/tikz-feynman/physics/siunitx/pgfplots/tikz-3dplot）、用户样式/坐标/节点/pic/foreach(含可选参数)/颜色/命令解析）
...
| `test_multitab` | 多标签页功能：创建/切换/关闭标签页、重复打开去重、关闭前未保存检查、编辑器过长行自动换行切换、元数据脏标记检测、**切换标签页元数据隔离与标题正确性**（切换更新右栏元数据、后台标签标题不被污染、未保存编辑跨切换保留） |
...
| `test_completer` | TikZ 补全词库完整性验证（选项/锚点/颜色/线型/交点/角度库关键字含重复检测）、detectContext 上下文检测（37 个测试用例，含 `\usetikzlibrary{}` 库补全）、PGF 键处理器、值提示（箭头/图案/角度/曲线）、`=` 后颜色值补全（`fill`/`draw`/`color` 提供完整调色板，非颜色键保留精选值）、`=` 后 key 名花括号/中括号感知提取、同行多命令取最内层命令、跨行上下文回溯、`\usetikzlibrary`/`\begin` 全管线库/环境补全、非法命令过滤（选项/环境/路径操作/CircuiTikZ 组件不作为 `\命令`）、CircuiTikZ 元件/形状与 CircuiTikZ 1.7.1 源码一致性校验（清除 `shape` 后缀垃圾与无效项）、标准 TikZ 形状/选项源码准确性校验（移除幻觉形状 `polygon`/`circle around`，保留 `bend at start`/`l_`/`american` 等合法项，`mirror`/`invert` 受 circuitikz 库门控）、数学函数源码一致性校验（清除 `cit`/`kil`/`vecilen`/`power` 等幻觉函数，补齐 `veclen`/`pow`/`atan2`/`Mod` 等）、装饰源码一致性校验（清除 `curly brace`/`Hilbert curve`，修正 core 装饰 lib 归属）、physics/siunitx 命令按宏包门控校验（未加载不提示、加载后提示）、pgfplots 键源码校验（`xtick distance`/`axis lines`/`colorbar`/`forget plot` 等，清除 `xstep`/`bar gap`/`histogram`）、tikz-cd/tkz-euclide/chemfig/tikz-feynman 宏包补全源码一致性校验（真实命令/样式齐全且按环境或宏包门控，清除 `obj`/`tweak`/`tkzDrawTriangle` 等无效项）、空列表模型清空 |
| `test_document_state` | 文档状态追踪：范围栈检测、库解析、用户样式（含空格名）/坐标/节点/pic名/foreach变量（含空格分隔多变量、>2个变量、可选方括号参数）/颜色/命令解析、环境名查询、片段库注入、`\usepackage{physics,siunitx,pgfplots,chemfig,tikz-feynman}` 激活对应补全库（16 个测试用例） |
